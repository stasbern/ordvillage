...
import layersData from '../public/samurai_layers.json';
...
const NFTBuilder = () => {
  ...
  Object.keys(layersData).forEach((folder) => {
    ...
    if (files.length > 0) {
      ...
      initialSelectedFiles[folder] = `samurai layers/${folder}/${randomFile}`;
  ...
  const handleFileChange = (folder, fileName) => {
    const filePath = `samurai layers/${folder}/${fileName}`;
  ...
  <div className="flex flex-row items-center justify-center flex-wrap">
    ...
    <div className="flex flex-col">
      ...
      {/* Export Button */}
      <button
        className="mt-4 px-4 py-2 bg-yellow-500 hover:bg-yellow-700 text-white font-bold rounded"
